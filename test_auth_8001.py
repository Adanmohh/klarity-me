...
# Test authentication flow on port 8001
print("1. Requesting OTP...")
response = requests.post(
    "http://localhost:8001/api/v1/auth-otp/request-otp",
...
response = requests.post(
    "http://localhost:8001/api/v1/auth-otp/verify-otp",
...
response = requests.get(
    "http://localhost:8001/api/v1/auth-otp/me",
    headers={"Authorization": f"Bearer {token}"}
)
print(f"   Status: {response.status_code}")
if response.status_code == 200:
